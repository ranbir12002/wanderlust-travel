...
import DestinationAdminActions from "@/components/admin/DestinationAdminActions";
import FeaturedCardsManager from "@/components/admin/FeaturedCardsManager";
...
export default async function AdminDashboard() {
  ...
  return (
    <div className="flex flex-col gap-12">
      ...
      <section>
        <FeaturedCardsManager />
      </section>
      {/* Destinations Section */}
      <section>
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h2 className="text-3xl font-black uppercase tracking-tight text-neutral-900">Manage Destinations</h2>
            <p className="text-neutral-500">Add or edit dynamic State/Country pages.</p>
          </div>
          <Link
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          {destinations.map((dest) => (
            <div key={dest.id} className="flex flex-col overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-sm transition-shadow hover:shadow-md">
              <div className="relative h-40 w-full bg-neutral-200">
                {dest.heroImage ? (
                  <Image src={dest.heroImage} alt={dest.title} fill className="object-cover" />
                ) : (
                  <div className="flex h-full w-full items-center justify-center text-xs font-bold text-neutral-400">NO IMAGE</div>
                )}
              </div>
              <div className="flex flex-1 flex-col p-4">
                <h3 className="line-clamp-1 font-bold">{dest.title}</h3>
                <p className="mb-4 text-xs font-medium text-neutral-500 capitalize">{dest.category} • Tag: {dest.tag}</p>
                <DestinationAdminActions destination={dest} />
              </div>
            </div>
          ))}
        </div>
